...
<?php
	...
	{	
		...
		foreach($html->find('a') as $link)
		{
				...
				if (!$r || mysqli_num_rows($r) > 0) { // Mat Need to Remove not R.
				mysqli_close($con);
				...
				{
					echo '<hr/>';
					echo '<a href="' . $link->href . '">' . $ptitle[0] . '</a><br/>';
					
					// DATE
					echo 'Extracted Date: ' . $avail[0] . "<br />";
					echo 'Cleaned Date: ';
					echo '<b>' . $db_date = date_clean($avail[0]);
					echo '</b>';
					...
					foreach($auth as $author)
					{
						...
						echo 'Author(s)/Actor(s) <font color=green><b>[Database-ready]</b></font>:<br/> ';
						// echo '<pre>';
					...
					echo '<br/>Final Title:<b> ' . $db_title . '</b>  <font color=green><b>[Database-ready]</b></font><br/>';
						...
						echo '<font color=blue>Inserted Successfully Into Database!</font><br/>';
						mysqli_query($con, "UPDATE $current_table SET find ='$findurl' WHERE title = '$db_title'");
						}}else{
						echo '<font color=red>Not Entered - Record Already Exists</font><br/>';
						}
						if(!$r){
						echo 'Not $R<br/>';
						}
