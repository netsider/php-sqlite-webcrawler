...
<form method="post"><table border="1" width="40%" bordercolor="green">
	...
	<tr><td align=center><input type="text" name="page" size="10" value="50"/></td><td>
	Read from: 
...
<?php
		...
		$r4 = mysqli_query($con, "SELECT * FROM `$current_source` LIMIT $page_counter");
		...
		{		
		    ...
		    mysqli_query($con, "DELETE FROM $current_source WHERE id = '$id'");
			...
			echo '</pre>';
				...
				if (strpos(implode("-", $avail),'released') === 0){
				echo 'Skipping - Not TBR!<br/>';
				continue;
				}
