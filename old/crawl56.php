<?php
	//AMAZON WEB CRAWLER
	//Designed on PHP 5.5, MySQL, Nginx (WINDOWS) 
	//ALL CONTENT WRITTEN BY RUSSELL ROUNDS UNLESS OTHERWISE SPECIFIED
	//7-11-2014
...
<form method="post"><table border="1" width="40%" bordercolor="green">
	...
	<tr><td align=center><input type="submit" name="plus" value="+" /> <input type="submit" name="minus" value="-" /></td>
		<td align=center><input type="submit" name="submit" value="Start/Begin" /></td></tr></table></form></center>
<?php
	if (isset($_POST['plus'])){	
	write('current.txt', $page_counter + 1);
	header("Location: crawl55.php");
	}
	...
	if (isset($_POST['submit'])){	
		...
		foreach($html->find('a') as $link)
		{
						...
						$con = mysqli_connect('127.0.0.1', 'updayte', 'dN6UFR8GC6K6LYax');
						...
						continue;
							...
							if (strpos(implode("-", $avail),'released') == true AND !strpos(implode("-", $avail),'been'))
							{
								...
								foreach ($platform as $p){ // INSERT a row for each $p in $platform
