<?php
	...
	function new_index($table){
		$conn = mysqli_connect('127.0.0.1', 'root', '', 'updayte');
		$q = "SELECT * FROM `links` WHERE paged = 1 LIMIT 1";
		$result = mysqli_query($conn, $q);
	...
	function crawl_all($target_url, $mhave, $mnot, $rurl, $index=0){
	...
	$array1 = crawl_all($url, $include, $exclude, $root_url, $index);
...
?>
	...
	<?php  
		echo 'Target URL: '; 
		echo '<span style="font-size:10px"><pre>';
		$url2 = new_index('links');
			print_r(base64_decode(unserialize(($url2))));
		echo '</pre></span>';
	?>
	</center>
...
<?PHP
		...
		mysqli_query($conn, "INSERT INTO `links`(original) VALUES ('$data')");
			}
