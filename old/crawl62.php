...
<form method="post"><table border="1" width="40%" bordercolor="green">
	...
	echo '</select>';
	?>
	-Or- <input type="submit" name="+last" value="Show Last X Entries"/> (X= <input type="text" name="x" size="3" value="15"> )</td> </tr>
	<tr><td align=center><input type="text" name="page" size="10" value="<?php echo $page_counter ?>"/></td><td align=center><input type="text" name="url" size="75" value="<?php echo $last_url ?>"/></td></tr>
			...
			}else
			{
				...
				if (strpos(implode("-", $avail),'released') == true AND !strpos(implode("-", $avail),'been'))
				{
					...
					{ 
						...
						{
							$result_array = array_push($db_title, $db_date, $db_source, $auth_db_array);
