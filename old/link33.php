...
?>
	...
	<tr>
	<td></td><?PHP if (isset($index)){echo 'Count: ' . $index; } ?><td><td><?PHP if (isset($badlinks)){echo 'Duplicates: ' . $badlinks; } ?></td></td>
	<td><?PHP if (isset($index)){echo 'Page_Counter: ' . $page_counter; } ?></td><td></td>
	</tr>
		<tr>
...
$url = 'http://www.amazon.com/movies';
	...
	function crawl($target_url, $mhave, $mnot, $rurl, $index){
		...
		foreach($html->find('a') as $link)
		{
			...
			if (strpos($link->href,'www.amazon.com') == 0 AND strpos($link->href,'http://') == 0) {
			...
			$link_href = $link->href;
			if (!in_array($url, $_SESSION, true) === false) {
			$array[$index]['url'] = $url;
	...
	echo $badlinks . ' duplicate links not added';
	write('bad.txt', $badlinks);
	return $array;
	...
	// Make sure it filters duplicates.
		...
		$data = $_SESSION['data'];
		$index = count($data);
		// $url =  $urla[$page_counter]['original'];
		$array1 = crawl($url, $include, $exclude, $root_url, $index);
